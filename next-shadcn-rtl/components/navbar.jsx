...
export default function Navbar() {
    ...
    const links = [
        ...
        { name: 'سوابق', href: '#stories' },
        { name: 'قیمت', href: '#pricing' },
        { name: 'اسناد', href: '#docs' },
    ];
                ...
                <div className='hidden items-center space-x-7 text-gray-800 dark:text-gray-200 md:flex'>
                    {links.map((link) => link.subLinks ? (
                        <div key={link.name} className='group relative' onMouseEnter={() => setOpenDropdown(link.name)} onMouseLeave={() => setOpenDropdown(null)}>
                            ...
                            <div className={`absolute top-6  inset-x-auto z-40 w-lg rounded-md border border-gray-100 text-gray-800 dark:text-gray-200 bg-gray-100 dark:bg-gray-900 p-3 shadow-lg transition-all duration-200 ease-in-out ${openDropdown === link.name ? 'visible translate-y-0 opacity-100' : 'invisible -translate-y-2 opacity-0'}`}>
